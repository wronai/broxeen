use anyhow::Result;
use opencv::{
    core::{Mat, Size, Scalar, Point, CV_8UC1, BORDER_DEFAULT, Vector},
    imgproc::{self, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE, MORPH_ELLIPSE, MORPH_CLOSE, MORPH_OPEN},
    video::{BackgroundSubtractorMOG2, create_background_subtractor_mog2},
    prelude::*,
};
...
pub struct MovingObject {
    /// Cropped image of the object (resized ≤500px longest edge)
    pub crop: Mat,
...
pub struct MotionDetector {
    ...
    max_area: f64,
    max_output_px: i32,
    // Morphological kernel (reused across frames)
    kernel: Mat,
}

impl MotionDetector {
    pub fn new(history: i32, var_threshold: f64, min_area: f64, max_area: f64, max_output_px: u32) -> Result<Self> {
        let subtractor = create_background_subtractor_mog2(
            history,
            var_threshold,
            false, // detectShadows=false → faster, no shadow pixels
        )?;

        // Elliptical kernel for morphological ops — removes noise while preserving shape
    ...
    pub fn process_frame(&mut self, frame: &Mat) -> Result<Vec<MovingObject>> {
        // ── 1. Scale frame down to 640px wide for fast processing ─────────
        ...
        self.subtractor.apply(&small, &mut fg_mask, -1.0)?;

        // ── 3. Threshold: only keep confident foreground (255), drop shadows ─
        let mut binary = Mat::default();
        opencv::core::threshold(&fg_mask, &mut binary, 200.0, 255.0, opencv::imgproc::THRESH_BINARY)?;

        // ── 4. Morphological close then open to remove noise ──────────────
        let mut closed = Mat::default();
        imgproc::morphology_ex(
            &binary, &mut closed, MORPH_CLOSE, &self.kernel,
            Point::new(-1, -1), 2, BORDER_DEFAULT, Scalar::default(),
        )?;
        let mut opened = Mat::default();
        imgproc::morphology_ex(
            &closed, &mut opened, MORPH_OPEN, &self.kernel,
            Point::new(-1, -1), 1, BORDER_DEFAULT, Scalar::default(),
        )?;
