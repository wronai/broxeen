...
function chatApiPlugin(): Plugin {
  ...
  function parseNmapHosts(out: string) {
    ...
    for (const m of out.matchAll(/Nmap scan report for\s+(?:\S+\s+\()?([\d.]+)/g)) {
      ...
      hosts.push({ ip, ports, hasCam: ports.some(p => [554,8554,8000,8080].includes(p)) });
    }
    return hosts;
  }
  ...
  function handleQuery(query: string, scope: string): { intent: string; content: Array<{type:string;data:string}> } {
    ...
    switch (intent) {
      ...
      case 'camera:onvif': {
        ...
        if (arp.length) {
          lines.push(`**ARP (${arp.length} hostów):**`);
          arp.forEach(e => {
            lines.push(`  📍 ${e.ip} [${e.mac ?? '??'}] (${e.state})`);
            if (isCam) lines.push(`     🎥 RTSP: \`rtsp://${e.ip}:554/stream\``);
          });
        }
        const nmapCmd = isCam
        ...
        if (nmapOut) {
          ...
          lines.push(`\n**nmap (${hosts.length} hostów):**`);
          hosts.forEach(h => {
            lines.push(`  ${h.hasCam ? '📷' : '🖥️ '} **${h.ip}**${h.ports.length ? ` ports: ${h.ports.join(',')}` : ''}`);
            if (h.hasCam) lines.push(`     🎥 RTSP: \`rtsp://${h.ip}:554/stream\``);
          });
        } else {
