...
describe('VoiceCommandsPlugin', () => {
  ...
  describe('execute', () => {
    ...
    it('should enable microphone', async () => {
      ...
      expect(mockedConfigStore.set).toHaveBeenCalledWith('mic_enabled', true);
    });
    ...
    it('should disable voice control', async () => {
      ...
      expect(mockedConfigStore.set).toHaveBeenCalledWith('mic_enabled', false);
      expect(mockedConfigStore.set).toHaveBeenCalledWith('stt_enabled', false);
    });
    ...
    it('should enable voice control', async () => {
      ...
      expect(mockedConfigStore.set).toHaveBeenCalledWith('mic_enabled', true);
      expect(mockedConfigStore.set).toHaveBeenCalledWith('stt_enabled', true);
    });

    it('should handle errors gracefully', async () => {
      const mockedConfigStore = vi.mocked(configStore);
      mockedConfigStore.set.mockRejectedValue(new Error('Config error'));
